...
/// </summary>
internal sealed class AetherytePositionDatabase
{
    ...
    internal AetherytePositionDatabase(Plugin plugin, IPluginLog log)
    {
        _plugin = plugin;
        _log = log;
        ...
        _log.Debug($"[AetheryteDB] Using config directory: {configDir}");
        _log.Debug($"[AetheryteDB] File path: {_filePath}");
        Load();
    }
    ...
    public AetherytePosition? GetPosition(uint aetheryteId)
    {
        ...
        _log.Debug($"[AetheryteDB] GetPosition({aetheryteId}): {(found && pos != null ? $"FOUND ({pos.Name})" : "NOT FOUND")}");
    ...
    public void RecordPosition(uint aetheryteId, string name, float x, float y, float z)
    {
        ...
        Save();
        _log.Debug($"[AetheryteDB] {(isNew ? "Recorded" : "Updated")} {name} (ID:{aetheryteId}) at ({pos.X:F1}, {pos.Y:F1}, {pos.Z:F1}) [total: {_positions.Count}]");
    }
    ...
    public void RemovePosition(uint aetheryteId)
    {
        ...
        {
            _log.Debug($"[AetheryteDB] Removed aetheryte ID {aetheryteId} from database - will appear as missing");
    ...
    public void ClearAllPositions()
    {
        ...
        _log.Debug($"[AetheryteDB] Cleared all {count} user positions - restored {_positions.Count} community defaults");
    }
    ...
    public void ClearAllPositionsForFreshScan()
    {
        ...
        try
        {
            if (File.Exists(_filePath))
            {
                ...
                _log.Debug($"[AetheryteDB] Deleted user file");
            }
        }
        catch (Exception ex)
        {
            _log.Debug($"[AetheryteDB] Failed to delete user file: {ex.Message}");
        }
        ...
        _log.Debug($"[AetheryteDB] Cleared all {count} positions - ready for fresh scan");
    }
    ...
    private void Load()
    {
        try
        {
            ...
            if (File.Exists(_filePath))
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    _log.Debug($"[AetheryteDB] Loaded {list.Count} user positions over {defaultCount} defaults = {_positions.Count} total");
                }
                else
                {
                    // User file is empty - user wants to start fresh, don't use defaults
                    _positions.Clear();
                    _log.Debug($"[AetheryteDB] Empty user file - starting fresh with no positions");
                    _log.Debug($"[AetheryteDB] WARNING: This means no community defaults are available!");
                }
            }
            else
            {
                _log.Debug($"[AetheryteDB] No user file - using {defaultCount} community defaults");
        ...
        _log.Debug($"[AetheryteDB] Initialization complete: {_positions.Count} total positions loaded");
        ...
        if (_positions.Count > 0)
        {
            ...
            foreach (var sample in samples)
                _log.Debug($"[AetheryteDB] Sample: {sample.Value.Name} (ID: {sample.Key}) = ({sample.Value.X:F1}, {sample.Value.Y:F1}, {sample.Value.Z:F1})");
        }
    }
    ...
    private void Save()
    {
        try
        {
            ...
            _log.Debug($"[AetheryteDB] Saved {_positions.Count} positions to {_filePath}");
        }
        catch (Exception ex)
        {
    ...
    public void ResetToCommunityDefaults()
    {
        try
        {
            ...
            _log.Debug($"[AetheryteDB] Reset to community defaults: {beforeCount} -> {afterCount} positions");

            // Save the fresh data to overwrite user file
            Save();
            _log.Debug($"[AetheryteDB] Overwrote user file with community defaults");
        }
        catch (Exception ex)
        {
    ...
    private void LoadCommunityData()
    {
        try
        {
            ...
            _log.Debug($"[AetheryteDB] Loaded {defaults.Count} community default aetheryte positions");
        }
        catch (Exception ex)
        {
...
internal sealed class AetherytePosition
{
