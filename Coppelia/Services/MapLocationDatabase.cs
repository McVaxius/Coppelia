...
/// </summary>
internal sealed class MapLocationDatabase
{
    ...
    internal MapLocationDatabase(Plugin plugin, IPluginLog log)
    {
        _plugin = plugin;
        _log = log;
    ...
    public MapLocationEntry? FindEntry(uint territoryId, float flagX, float flagZ)
    {
        ...
        foreach (var entry in _communityEntries)
        {
            ...
            if (xzDist <= 10.0)
            {
                if (entry.HasRealXYZ)
                {
                    _log.Debug($"[MapLocDB] Community hit: {entry.ZoneName} real=({entry.RealX:F1},{entry.RealY:F1},{entry.RealZ:F1}) dist={xzDist:F1}y");
                    return entry;
                }
                bestNoReal ??= entry;
            }
        ...
        foreach (var entry in _userEntries)
        {
            ...
            if (xzDist <= 10.0)
            {
                if (entry.HasRealXYZ)
                {
                    _log.Debug($"[MapLocDB] User hit: {entry.ZoneName} real=({entry.RealX:F1},{entry.RealY:F1},{entry.RealZ:F1}) dist={xzDist:F1}y");
                    return entry;
                }
                bestNoReal ??= entry;
            }
        ...
        if (bestNoReal != null)
            _log.Debug($"[MapLocDB] Found entry but no RealXYZ: {bestNoReal.ZoneName} flag=({bestNoReal.FlagX:F1},{bestNoReal.FlagZ:F1})");
    ...
    public void RecordLocation(uint territoryId, string zoneName, string mapName, float flagX, float flagY, float flagZ, float realX, float realY, float realZ)
    {
        ...
        foreach (var entry in _userEntries)
        {
            ...
            if (Math.Sqrt(dx * dx + dz * dz) <= 10.0)
            {
                if (entry.HasRealXYZ)
                {
                    _log.Debug($"[MapLocDB] User file already has entry, skipping");
                    return;
                }
                ...
                SaveUser();
                _log.Debug($"[MapLocDB] Updated user entry with RealXYZ: real=({realX:F1},{realY:F1},{realZ:F1})");
                return;
            }
        }
        ...
        SaveUser();
        _log.Debug($"[MapLocDB] Recorded user location: {zoneName} T{territoryId} flag=({flagX:F1},{flagZ:F1}) real=({realX:F1},{realY:F1},{realZ:F1}) [user total: {_userEntries.Count}]");
    }
    ...
    public void PopulateFromTreasureSpot(IDataManager dataManager)
    {
        try
        {
            ...
            _log.Debug($"[MapLocDB] TreasureSpot: {totalSpots} spots found, {newSpots} new (not in community/user files)");
        }
        catch (Exception ex)
        {
            _log.Error($"TreasureSpot population failed: {ex.Message}");
            _log.Debug($"[MapLocDB] TreasureSpot failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    public async Task<bool> DownloadCommunityDataAsync()
    {
        ...
        try
        {
            ...
            if (remoteEntries == null || remoteEntries.Count == 0)
            {
                ...
                _log.Debug("[MapLocDB] Download: no data received");
                return false;
            }
            ...
            if (!SaveCommunity())
            {
                ...
                _log.Debug("[MapLocDB] Download failed: could not save replaced community data");
                return false;
            }

            LastDownloadResult = $"OK: replaced {_communityEntries.Count} community locs from GitHub";
            _log.Debug($"[MapLocDB] Download complete: replaced {_communityEntries.Count} community locs from GitHub");
            return true;
        }
        catch (Exception ex)
        {
            LastDownloadResult = $"Error: {ex.Message}";
            _log.Debug($"[MapLocDB] Download failed: {ex.GetType().Name}: {ex.Message}");
            return false;
        }
        finally
        {
        ...
        if (File.Exists(_legacyFilePath) && !File.Exists(_userFilePath))
        {
            try
            {
                ...
                if (legacy != null && legacy.Count > 0)
                {
                    ...
                    _log.Debug($"[MapLocDB] Migrated {legacy.Count} entries from legacy MapLocations.json -> UserMapLocations.json");
                }
    ...
    private List<MapLocationEntry> LoadFile(string path, string label)
    {
        try
        {
            if (File.Exists(path))
            {
                ...
                _log.Debug($"[MapLocDB] Loaded {entries.Count} {label} entries");
                return entries;
            }
            _log.Debug($"[MapLocDB] No {label} file found");
        }
        catch (Exception ex)
        {
...
internal sealed class MapLocationEntry
{
